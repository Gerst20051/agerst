...
for (var i = 0; i < sad.length; i++) {
	if (sad[i] && hasClass(sad[i], 'cmnt_like_link')) {
		var sad_likebutton = sad[i].getElementsByTagName('span');
		for (var j = 0; j < sad_likebutton.length; j++) {
			if (sad_likebutton[j] && hasClass(sad_likebutton[j], 'default_message') && sad_likebutton[j].style.display != 'none' && sad_likebutton[0].innerHTML == 'Unlike') happy.push(sad[i]);
		}
	}
}
